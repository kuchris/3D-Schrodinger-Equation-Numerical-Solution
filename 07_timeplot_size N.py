...
ax1.plot(x, exponential(x, *pars), linestyle='--', linewidth=2, color='black')
plt.title("Computation time for different N")
plt.ylabel(r'Second $s$')
plt.xlabel(r'Size of matrix $N$')
...
a=pars[0]
b=pars[1]
at = AnchoredText(r"$t={:.2f}*exp({:.2f}*N)$" "\n" "$k=1$".format(a, b), prop=dict(size=15), frameon=True, loc=4)
